...
const createTask = asyncHandler(async (req, res) => {
  try {
    ...
    let { title, team, group, stage, date, priority, assets, links, description } =
      req.body;

    // If group is assigned, fetch group members and assign to team
    if (group) {
      const groupData = await import("../models/Group.js").then(mod => mod.default.findById(group).select("members"));
      if (groupData) {
        team = groupData.members;
      }
    }
